...
final class Fsck {
    ...
    for (final Query query : queries) {
      ...
      while ((rows = scanner.nextRows().joinUninterruptibly()) != null) {
        for (final ArrayList<KeyValue> row : rows) {
          ...
          if (prev == null) {
            prev = new Seen((base_time - 1)*Const.ONE_SEC_IN_MILLIS, row.get(0));
            seen.put(key, prev);
          }
          for (final KeyValue kv : row) {
            ...
            if (qual.length < 4) {
              ...
              prev.setTimestamp(base_time*Const.ONE_SEC_IN_MILLIS + last_delta);
              prev.kv = kv;
            ...
            final long timestamp = base_time*Const.ONE_SEC_IN_MILLIS + delta;
            if (value.length > 8) {
              errors++;
            ...
            if (timestamp <= prev.timestamp()) {
              ...
              if (fix) {
                ...
                final long new_base_time_seconds = new_base_time / Const.ONE_SEC_IN_MILLIS;
                Bytes.setInt(newkey, (int) new_base_time_seconds, metric_width);
  ...
  private static final class Seen {
    /** A 64-bit unsigned integer that holds a UNIX timestamp in milliseconds.  */
    ...
    /** Returns the UNIX timestamp (in milliseconds) as a 64-bit unsigned int.  */
    public long timestamp() {
      return timestamp;
    }

    /** Updates the UNIX timestamp (in milliseconds) with a 64-bit unsigned int.  */
