...
final class DumpSeries {
  ...
  private static final SimpleDateFormat formatter = new SimpleDateFormat("dd MMM yyyy HH:mm:ss:SSS Z");
        ...
        for (final ArrayList<KeyValue> row : rows) {
          ...
          if (!importformat) {
            buf.append(Arrays.toString(key))
              ...
              .append(metric)
              .append(' ')
              .append(base_time * Const.ONE_SEC_IN_MILLIS)
              .append(" (").append(date(base_time * Const.ONE_SEC_IN_MILLIS)).append(") ");
            try {
  ...
  private static void formatKeyValue(final StringBuilder buf,
    ...
    for (int i = 0; i < nvalues; i++) {
      ...
      final int delta = ((0xFFFFFFFF & qual) >>> Internal.FLAG_BITS);
      if (importformat) {
        buf.append(base_time * Const.ONE_SEC_IN_MILLIS + delta).append(' ');
      } else {
      ...
      if (importformat) {
        ...
        buf.append('\t')
           .append(base_time * Const.ONE_SEC_IN_MILLIS + delta)
           .append(" (").append(date(base_time * Const.ONE_SEC_IN_MILLIS + delta)).append(')');
      }
