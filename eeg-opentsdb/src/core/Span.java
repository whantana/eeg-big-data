...
final class Span implements DataPoints {
  ...
  void addRow(final KeyValue row) {
    ...
    if (rows.size() != 0) {
      ...
      // the last RowSeq.
      long last_bt = last.baseTime() * Const.ONE_SEC_IN_MILLIS;
      long last_ts_in_row = lastTimestampInRow(metric_width, row);
      long time_delta = last_ts_in_row - last_bt;
      boolean canTimeDeltaFit = RowSeq.canTimeDeltaFit(time_delta);
      if (canTimeDeltaFit) { 
	     last.addRow(row);
	     return;
       }
  ...
  static long lastTimestampInRow(final short metric_width,
    ...
    return base_time * Const.ONE_SEC_IN_MILLIS + last_delta;
  }
  ...
  final class DownsamplingIterator
    ...
    public DataPoint next() {
      ...
      do {
        ...
        //LOG.debug("Downsampling @ time " + current_row.timestamp() + " end: " + time + " npoints: " + npoints);
      ...
      if (row_index != saved_row_index) {
        //LOG.debug("restoring to " + saved_row_index);
