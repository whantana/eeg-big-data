...
final class CompactionQueue extends ConcurrentSkipListMap<byte[], Boolean> {
  ...
  public Deferred<ArrayList<Object>> flush() {
    ...
    final long now_in_seconds = now / Const.ONE_SEC_IN_MILLIS;
    final long max_timespan_in_seconds = Const.MAX_TIMESPAN/Const.ONE_SEC_IN_MILLIS;
    final long cut_off = now_in_seconds - max_timespan_in_seconds - 1;
    return flush(cut_off, Integer.MAX_VALUE);
  }
  ...
  private Deferred<Object> compact(final ArrayList<KeyValue> row,
    ...
    if (row.size() <= 1) {
      if (row.isEmpty()) {  // Maybe the row got deleted in the mean time?
        ...
        if (qual.length == 4 && floatingPointValueToFix(qual[3], val)) {
          // Fix up old, incorrectly encoded floating point value.
          final byte[] newval = fixFloatingPointValue(qual[3], val);
          final byte q = fixQualifierFlags(qual[3], newval.length);
          final byte[] newqual = new byte[] { qual[0], qual[1], qual[2], q};
    ...
    final KeyValue compact;
    {
      ...
      for (int i = 0; i < nkvs; i++) {
        ...
        if (len != 4) {
          ...
          final int delta = ((Bytes.getInt(qual) & 0xFFFFFFFF) >>> Const.FLAG_BITS);
  ...
  static KeyValue complexCompact(final ArrayList<KeyValue> row,
    ...
    for (int i = 0; i < ncells; i++) {
      ...
      final int delta = (short) ((Bytes.getInt(cell.qualifier) & 0xFFFFFFFF)
                                   >>> Const.FLAG_BITS);
      ...
      while (true) {
        try {
          ...
          if (last_flush - now > Const.MAX_TIMESPAN  // (1)
              || size > maxflushes) {                // (2)
            flush((now - Const.MAX_TIMESPAN - 1)/Const.ONE_SEC_IN_MILLIS, maxflushes);
