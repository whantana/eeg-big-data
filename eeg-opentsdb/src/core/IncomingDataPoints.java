...
final class IncomingDataPoints implements WritableDataPoints {
  ...
  private long updateBaseTime(final long timestamp) {
    ...
    final long base_time = (timestamp - (timestamp % Const.MAX_TIMESPAN)) / Const.ONE_SEC_IN_MILLIS;
    ...
    tsdb.scheduleForCompaction(row, base_time);
    return base_time;
  }
  ...
  private Deferred<Object> addPointInternal(final long timestamp, final byte[] value,
    ...
    if (size > 0) {
      base_time = baseTime();
      final long last_ts = timestamp(size - 1);
      if (timestamp <= last_ts) {
        throw new IllegalArgumentException("New timestamp=" + timestamp
            ...
            + " to " + this);
      } else if (timestamp - base_time * Const.ONE_SEC_IN_MILLIS >= Const.MAX_TIMESPAN) {
    ...
    final int qualifier = (int) ((timestamp - base_time * Const.ONE_SEC_IN_MILLIS) << Const.FLAG_BITS
                                     | flags);
  ...
  public long timestamp(final int i) {
    checkIndex(i);
    return (baseTime() * Const.ONE_SEC_IN_MILLIS) + (delta(qualifiers[i]) & 0xFFFFFFFF);
  }
