...
final class RowSeq implements DataPoints {
  ...
  void addRow(final KeyValue row) {
    ...
    for (int i = 0; i < len; i += 4) {
      ...
      qualifier = ((time_delta << Const.FLAG_BITS) | (qualifier & Const.FLAGS_MASK));
  ...
  static boolean canTimeDeltaFit(final long time_delta) {
    ...
    //LOG.debug("Long.SIZE - Const.FLAG_BITS " + (Long.SIZE - Const.FLAG_BITS));
    //LOG.debug("1 << (Long.SIZE - Const.FLAG_BITS) " + (long) (1 << (Long.SIZE - Const.FLAG_BITS)));
  ...
  int baseTime() {
    return (int) Bytes.getUnsignedInt(key, tsdb.metrics.width());
  }
  ...
  public long timestamp(final int i) {
    ...
    return baseTime()*Const.ONE_SEC_IN_MILLIS + (Bytes.getInt(qualifiers, i * 4) >>> Const.FLAG_BITS);
  }
  ...
  public String toString() {
    ...
    buf.append("RowSeq(")
       ...
       .append(base_time)
       .append(" (")
       .append(base_time > 0 ? new Date(base_time * Const.ONE_SEC_IN_MILLIS) : "no date")
       .append("), [");
  ...
  final class Iterator implements SeekableView, DataPoint {
    ...
    public DataPoint next() {
      ...
      //java.text.SimpleDateFormat formatter = new //java.text.SimpleDateFormat("dd MMM yyyy HH:mm:ss:SSS Z");
      //LOG.debug("next -> now=" + toStringSummary() + " " + timestamp() + " " + formatter.format(new Date(timestamp())));
      return this;
    }
    ...
    public void seek(final long timestamp) {
      if (timestamp < 0) {  // negative or not 32 bits
	  ...
	  //java.text.SimpleDateFormat formatter = new //java.text.SimpleDateFormat("dd MMM yyyy HH:mm:ss:SSS Z");
      //LOG.debug("seek to " + timestamp + " " + formatter.format(new Date(timestamp))  + " -> now=" + toStringSummary());
    }
    ...
    public long timestamp() {
      ...
      return base_time*Const.ONE_SEC_IN_MILLIS + ((qualifier & 0xFFFFFFFF) >>> Const.FLAG_BITS);
    }
    ...
    long saveState() {
      ...
      //LOG.debug("qual_index " + qual_index);
      //LOG.debug("(qual_index << 32) " + ((long) qual_index << 32));
      //LOG.debug("(value_index & 0xFFFFFFFF) " +  ((long)value_index & 0xFFFFFFFF));
      //LOG.debug("ret " + (((long) qual_index << 32) | ((long) value_index & 0xFFFFFFFF)));
    ...
    void restoreState(long state) {
      ...
      //LOG.debug("value_index " + value_index);
      //LOG.debug("qual_index " + qual_index);
    }
    ...
    long peekNextTimestamp() {
      return base_time*Const.ONE_SEC_IN_MILLIS
