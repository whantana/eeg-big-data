...
final class TsdbQuery implements Query {
  ...
  /** Start time (UNIX timestamp in milliseconds) on 64 bits ("unsigned" int). */
  private long start_time = UNSET;

  /** End time (UNIX timestamp in milliseconds) on 64 bits ("unsigned" int). */
  private long end_time = UNSET;

  /** ID of the metric being looked up. */
  private byte[] metric;

  /** Do we need to be exact in the start/end time limit **/
  ...
  public DataPoints[] run() throws HBaseException {
	TreeMap<byte[], Span> spans = findSpans();
    return groupByAndAggregate(spans);
  }
  ...
  private DataPoints[] groupByAndAggregate(final TreeMap<byte[], Span> spans) {
    ...
    long startTime = (this.exact) ? getStartTime() : getScanStartTime() * Const.ONE_SEC_IN_MILLIS;
    long endTime = (this.exact) ? getEndTime() : getScanEndTime() * Const.ONE_SEC_IN_MILLIS;
    ...
    for (final Map.Entry<byte[], Span> entry : spans.entrySet()) {
      ...
      if (thegroup == null) {
        thegroup = new SpanGroup(tsdb, startTime, endTime,
  ...
  Scanner getScanner() throws HBaseException {
	...
	int scanStartTime = (int) (getScanStartTime());
	int scanEndTime = (int) ((end_time == UNSET) ? (-1) : (getScanEndTime()));
         
	Bytes.setInt(start_row, scanStartTime, metric_width);
    ...
    *  Omitting the millisecond digits  
    */
  private long getScanStartTime() {
   ...
   //final long ts = (getStartTime() - (Const.MAX_TIMESPAN * 2 - sample_interval*1000));
    ...
    final long start_time = getStartTime();
    final long ts = ((start_time - (start_time % Const.MAX_TIMESPAN)) / Const.ONE_SEC_IN_MILLIS);
    return ts > 0 ? ts : 0;
  }

  /** 
   *  Returns the UNIX timestamp from which we must stop scanning. 
   *  Omitting the millisecond digits  
   */
  private long getScanEndTime() {
	...
	//final long ts = (getEndTime() + (Const.MAX_TIMESPAN + 1000) + sample_interval*1000);
	
	final long end_time = getEndTime();
	final long ts = ((end_time - (end_time % Const.MAX_TIMESPAN)) / Const.ONE_SEC_IN_MILLIS) + 
	      Const.MAX_TIMESPAN / Const.ONE_SEC_IN_MILLIS;
	return ts;
  }
