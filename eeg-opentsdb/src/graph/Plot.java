...
public final class Plot {
  ...
  /** Start time (UNIX timestamp in milliseconds) on 64 bits ("unsigned" long). */
  private final long start_time;

  /** End time (UNIX timestamp in milliseconds) on 64 bits ("unsigned" long). */
  ...
  /**
   * Number of milliseconds of difference to apply in order to get local time.
   ...
   */
   public Plot(final long start_time, final long end_time, TimeZone tz) {
    ...
    this.utc_offset = (long) (tz.getOffset(System.currentTimeMillis()));
  }
  ...
  public int dumpToFiles(final String basepath) throws IOException {
    ...
    for (int i = 0; i < nseries; i++) {
      ...
      try {
        for (final DataPoint d : datapoints.get(i)) {
          ...
          long unix_timestamp = unixTimestampInSeconds(offsetedTimestamp(ts));
          int seconds = secondsOfTimestamp(offsetedTimestamp(ts));
          int millis  = millisOfTimestamp(offsetedTimestamp(ts));
          String dataEntry = unix_timestamp + "-" + String.format("%02d",seconds) +
             "." + String.format("%03d",millis);
          datafile.print(dataEntry);
          datafile.print(' ');
          if (d.isInteger()) {
        ...
        datafile.close();
        LOG.info("Wrote datafile to " + datafiles[i]);
      }
    }
    ...
    if (npoints == 0) {
      ...
      params.put("yrange", "[-10:10]");  // Doesn't matter what values we use.
    }
  ...
  private void writeGnuplotScript(final String basepath,
    ...
    try {
      ...
      final long xrange_start = unixTimestampInSeconds(offsetedTimestamp(start_time));
      long xrange_end = unixTimestampInSeconds(offsetedTimestamp(end_time));
      if((xrange_end - xrange_start) <= 0) xrange_end++;
      gp.append("\n"
                + "set xdata time\n"
                + "set timefmt \"%s-%S\"\n"
                //+ "set xtic rotate\n"
                ...
                + "set xrange [\"")
        .append(String.valueOf(xrange_start))
        .append("\":\"")
        .append(String.valueOf(xrange_end))
        .append("\"]\n");
  ...
  /**
   * Returns the timestamp added with the utc_offset
   * @param ts timestamp. 
   * @return the timestamp added with the utc_offset. 
   */
  private long offsetedTimestamp(final long ts) {
    return ts + TimeZone.getDefault().getOffset(ts);  	
  }
  
  /**
   * Returns the classic unix timestamp (epoch) in seconds.
   * @param ts timestamp (milliseconds). 
   * @return the timestamp added with the utc_offset. 
   */
  private long unixTimestampInSeconds(final long ts) {
	return (ts / Const.ONE_SEC_IN_MILLIS);
  }
  
  /**
   * Returns seconds (0-60) from the timestamp.
   * @param ts timestamp (milliseconds).
   * @return seconds (0-60) from the timestamp.
   */
  private int secondsOfTimestamp(final long ts) {
  ...
  /**
   * Returns milliseconds (0-999) from the timestamp.
   * @param ts timestamp (milliseconds).
   * @return milliseconds (0-999) from the timestamp.
   */
  private int millisOfTimestamp(final long ts) {
    return (int) (ts % Const.ONE_SEC_IN_MILLIS);
  } 
  ...
  private String xFormat() {
	long start_time_seconds = unixTimestampInSeconds(start_time);
	long end_time_seconds = unixTimestampInSeconds(end_time);
    long timespan = (end_time_seconds - start_time_seconds);
    
    if (timespan < 60) { // 1sec
	  return "%H:%M:%02.3S";
    } else if (timespan < 2100) {  // 35m
      return "%H:%M:%S";
